"""
description: Utilitario para substituir tags no texto pelos campos recebidos.
...
def replace_tags(text: str, fields: dict, default_value: str = '') -> str:
    ...
    # Validacao de campos
    if not text or not fields or \
       not _.is_string(text) or \
       not _.is_dict(fields):
        return ''
    # Extrair tags
    ...
    tags_uniq = _.uniq(tags_sorted)
    # Texto que será substituido
    text_final = text
    # Substituir
    for tag in tags_uniq:
        regex = REGEX_REPLACE_TAGS % tag
        text_final = re.sub(re.compile(regex), str(_.get(fields, tag, default_value)), str(text_final))
    # Retorno
    return text_final


def interpolation(text: str = None, fields: dict = None, is_json: bool = False) -> str:
    """
    Executa a substituição de tags (Ex.: "{{campo}}") dentro do texto a partir de um dicionario.
...
def unicode_normalize(text: str) -> str:
    ...
    if not isinstance(text, str): 
        return '' 
    import unicodedata
    text_new = text.encode('unicode-escape', 'ignore').decode('unicode-escape', 'ignore') 
    text_new = unicodedata.normalize("NFKD", text_new) 
    return text_new


def search_text(words: str, text: str, case_sensitive: bool = True, no_latin : bool = False,
                ignore_spaces: bool = False) -> bool:
    """
